[{:name "re-matcher", :desc "creates iterable regex"}
 ...
 {:name "clojure.set/join" :desc "joins sets of maps by common key in maps"}
 ...
 {:name "require" :desc "takes a quoted list b/c it's a fn and not a macro like ns"}
 ...
 {:name "pmap" :desc "parallel map, if process time is low, chunk up processes using detection-all"}
 {:name "promise" :desc "declaratively set with deliver, blocks"}
 {:name "future" :desc "Thread.new with cached result + optional timeout"}
 {:name "realized?" :desc "checks on delays, promises, futures, lazyseqs"}
 {:name "clojure.zip/zipper" :desc "[children? children create-branch]"}
 ...
 {:name "merge-with" :desc "merges maps with same keys given a function to merge values"}
 ...
 {:name "next" :desc "produces nil for empty cases, less lazy as it checks head of nonempty tail"}
 {:name "rest" :desc "produces () for empty cases, max lazy"}
 {:name "conj" :desc "list + vector adds elements in different places b/c of efficiency"}
